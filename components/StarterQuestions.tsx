      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {isEditing ? (
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <IconLoader2 size={13} style={{ animation: 'sq-spin 1s linear infinite', flexShrink: 0, color: T.fg4 }} />
...
export default function StarterQuestions({
  ...
  const [manualQuestions, setManualQuestions] = useState<Question[]>(
    initialQuestions.map((text, i) => ({ id: `q-${i}`, text, source: 'manual' as const }))
  )
  ...
  // Sync questions back to parent
  useEffect(() => {
    onChange?.(displayedQuestions.map(q => q.text))
  }, [manualQuestions, aiQuestions, crOn]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function StatusChip() {
    ...
    if (crStatus === 'generating') return (
      <span style={{ ...base, background: T.primary100, color: '#5C53C0' }}>
        <IconLoader2 size={11} style={{ animation: 'sq-spin 1s linear infinite' }} /> Generating…
      </span>
    )
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes sq-shimmer{0%{background-position:-600px 0}100%{background-position:600px 0}}
        @keyframes sq-fadeSlideIn{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:translateY(0)}}
      `}</style>
        ...
        <DraggingCtx.Provider value={anyDragging}>
          ...
          >
            <div style={{
              ...
              marginBottom: 12,
              overflow: 'visible',
              animation: crOn ? 'sq-fadeSlideIn 0.28s cubic-bezier(0.2,0.7,0.3,1) both' : undefined,
            }}>
